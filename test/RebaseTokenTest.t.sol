...
contract RebaseTokenTest is Test {
    ...
    function testDepositLinear(uint256 amount) public {
        ...
        console2.log("starting balance :", startBalance);
    ...
    function testRedeemAfterTimePassed(uint256 depositAmount, uint256 time) public {
        ...
        vm.prank(owner);
        vm.deal(owner, 3 * (balanceAfterSomeTime - depositAmount));
        addRewardToVault(2 * (balanceAfterSomeTime - depositAmount));

        vm.prank(user);
        vault.redeem(type(uint256).max);

        uint256 ethBalance = address(user).balance;
        assertApproxEqAbs(balanceAfterSomeTime, ethBalance, 1);
    ...
    function testInterestRateCanOnlyDecrease(uint256 newInterestRate) public {
        ...
        rebaseToken.setInterestRate(newInterestRate);
        assertEq(initalInterestRate, rebaseToken.getInterestRate());
    }
}
